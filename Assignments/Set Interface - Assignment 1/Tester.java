...
class Tester {
    ...
    public static Set<Student> findUnique(List<Student> students) {
        return new HashSet<>(students); // HashSet automatically removes duplicates by email
    }

    //  Get students who applied for both events
    public static Set<Student> findDuplicates(List<Student> students) {
        ...
        for (Student s : students) {
            if (!seen.add(s)) { // if already exists → duplicate
                duplicates.add(s);
            }
        }
        return duplicates;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Students who have submitted nominations for both events:");
