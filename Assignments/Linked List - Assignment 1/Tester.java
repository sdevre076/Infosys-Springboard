...
class Tester {
    ...
    public static void shiftListLeft(LinkedList linkedList, int n) {
        if (linkedList == null || linkedList.getHead() == null) return;
        ...
        // rotate left n times by moving head to tail
        for (int i = 0; i < n; i++) {
            String data = linkedList.getHead().getData();
            linkedList.addAtEnd(data);
            linkedList.delete(data); // deletes first occurrence (head)
        }
    }
    ...
    public static void shiftListRight(LinkedList linkedList, int n) {
        ...
        // right shift by n = left shift by (len - n)
        shiftListLeft(linkedList, len - n);
    }
}
